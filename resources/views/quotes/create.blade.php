                ...
                <script>
                    document.addEventListener('DOMContentLoaded', function() {
                        Swal.fire({
                            ...
                            html: `{!! '<ul class="text-left text-sm">'.collect($errors->all())->map(fn($e)=>'<li class="text-red-600">• '.e($e).'</li>')->implode('').'</ul>' !!}`,
                            confirmButtonColor: '#EF4444'
                        });
                    });
                </script>
    ...
    @push('scripts')
        <script>
            ...
            function addToQuote(button) {
                ...
                if (existing) {
                    existing.quantity++;
                    toast('info', `Cantidad actualizada: ${existing.quantity}`);
                } else {
                    quoteItems.push({ product_id: productId, name: productName, price: productPrice, quantity: 1 });
                ...
                if (newQuantity <= 0) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            document.querySelectorAll('.quantity-input')[index].value = quoteItems[index]?.quantity ?? 1;
                        }
                    });
                } else {
            ...
            function updateQuoteDisplay() {
                ...
                if (taxEnabled) document.getElementById('taxDisplay').textContent = '$' + tax.toLocaleString('es-CO');
                document.getElementById('totalDisplay').textContent = '$' + total.toLocaleString('es-CO');
            }

            // Búsqueda (DOMContentLoaded por si el script se pushea al final)
            ...
            function saveCustomer() {
                ...
                if (!formData.get('name')) {
                    Swal.fire({ icon: 'error', title: 'Error', text: 'El nombre del cliente es requerido', confirmButtonColor: '#EF4444' });
                    return;
                }
                Swal.fire({ title: 'Guardando cliente...', html: 'Por favor espera', allowOutsideClick: false, didOpen: () => Swal.showLoading() });
                ...
                .then(data => {
                    if (data?.success) {
                        ...
                        Swal.fire({ icon: 'error', title: 'Error', text: data?.message || 'Error al crear cliente', confirmButtonColor: '#EF4444' });
                    }
                })
                .catch(() => {
                    Swal.fire({ icon: 'error', title: 'Error', text: 'Error al crear cliente. Por favor intenta de nuevo.', confirmButtonColor: '#EF4444' });
                });
            }
            ...
            document.getElementById('quoteForm').addEventListener('submit', function(e) {
                if (quoteItems.length === 0) {
                    e.preventDefault();
                    Swal.fire({ icon: 'warning', title: 'Atención', text: 'Debes agregar al menos un producto a la cotización', confirmButtonColor: '#F59E0B' });
                }
            });
            ...
            document.addEventListener('keydown', function(e) {
                if (e.key === 'Escape') {
                    ...
                    if (!modal.classList.contains('hidden')) closeCustomerModal();
                }
            });
        </script>
    @endpush
