...
<div
    x-data="{
        isOpen: $wire.entangle('showModal'),
        recording: false,
        mediaRecorder: null,
        audioChunks: [],
        transcript: $wire.entangle('voiceTranscript').live ?? '',
        processing: $wire.entangle('voiceProcessing').live ?? false,
        extractedData: $wire.entangle('voiceExtractedData').live ?? null,
        ...
        async startRecording() {
            ...
            try {
                ...
                // Detectar mimeType compatible (Safari/iOS suele preferir audio/mp4)
                ...
                this.mediaRecorder.onstop = async () => {
                    // Usa el mismo mimeType para el Blob final si existe
                ...
                if (error.name === 'NotAllowedError') {
                    ...
                    Swal.fire({
                        title: 'Error',
                        text: 'No pudimos acceder al micrófono. Revisa permisos del navegador.',
                        icon: 'error'
                    });
                }
            }
    ...
    style="display: none;"
>
    <div x-show="isOpen" x-transition @click="$wire.close()" class="fixed inset-0 bg-black bg-opacity-75"></div>

    <div class="flex items-center justify-center min-h-screen p-4">
        <div x-show="isOpen" x-transition @click.stop class="relative bg-white rounded-3xl shadow-2xl max-w-2xl w-full p-8">
            ...
            <div class="flex items-center justify-between mb-6">
                ...
                <div class="flex gap-2">
                    ...
                    <button @click="$wire.close()" class="text-gray-400 hover:text-gray-600" title="Cerrar">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="flex flex-col items-center py-8">
                ...
                <p class="mt-4 text-lg font-bold text-gray-700"
                   x-text="recording ? '🔴 Grabando...' : (processing ? '⏳ Procesando...' : '👆 Presiona para grabar')"></p>
            </div>
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <p class="font-bold text-green-900"
                           x-text="extractedData?.precio ? '$' + Number(extractedData.precio).toLocaleString() : '-'"></p>
                    </div>
                    <div>
                        <p class="text-xs text-green-700">Costo:</p>
                        <p class="font-bold text-green-900"
                           x-text="extractedData?.costo ? '$' + Number(extractedData.costo).toLocaleString() : '-'"></p>
                    </div>
...
@script
<script>
    // Livewire events (toast de éxito / error)
